...
## Приложение. VirtualBox — экран на весь монитор

Если Ubuntu в VirtualBox запускается в маленьком окне и не растягивается:

```bash
# 1. ставим зависимости для сборки
sudo apt install -y build-essential dkms linux-headers-$(uname -r)

# 2. VirtualBox: «Устройства» → «Вставить образ диска Guest Additions…»

# 3. монтируем диск и запускаем установщик
...
После перезагрузки — нажать **Правый Ctrl + F** или **«Вид» → «Режим интеграции экрана»**. Экран будет подстраиваться под размер окна.

**Если препод спросит:** Guest Additions — это набор драйверов и утилит от VirtualBox для гостевой ОС. Без них базовая видеокарта работает в минимальном разрешении, мышь не интегрируется с хостом, нет буфера обмена. С дополнениями — фуллскрин, общий буфер обмена, перетаскивание файлов.
